        ...
        function fillTimeRanges(event) {
            ...
            function addSingleLine(e, i) {
                ...
                tr.append($('<td>').append(e.label).append($('<input type="text">').attr('id', e.prefix + '_val_' + i).attr('value', c_profile[e.array][i].value).attr('class', e.prefix + '_value')
                        .attr('onChange', 'recalXEins("'+ e.prefix +'", ' + i +')')));
                if (e.prefix === 'pe_ic') {
                    var newVal = parseFloat(c_profile[e.array][i].value) !== 0 ? (10 / parseFloat(c_profile[e.array][i].value)).toFixed(2) : 0;
                    tr.append($('<td>').append('1ХЕ:Инс').append($('<input type="text">').attr('id', e.prefix + '_val_xe_' + i).attr('value', newVal)
                            .attr('onChange', 'recalinsXE("'+ e.prefix +'", ' + i +')')));
                }
                var icons_td = $('<td>').append($('<img>').attr('class', 'addsingle').attr('style', 'cursor:pointer').attr('title', translate('Add new interval before')).attr('src', icon_add)
                        .attr('array', e.array).attr('pos', i));
...
function recalXEins(prefix, i) {
    if (!recalcInProcess && $$('#' + prefix + '_val_' + i).val() != null && $$('#' + prefix + '_val_' + i).val() != 0) {
        recalcInProcess = true;
        $$('#' + prefix + '_val_xe_' + i).val((10 / parseFloat($$('#' + prefix + '_val_' + i).val().replace(/\,/g, '.'))).toFixed(2));
        recalcInProcess = false;
    }
}

function recalinsXE(prefix, i) {
    if (!recalcInProcess && $$('#' + prefix + '_val_xe_' + i).val() != null && $$('#' + prefix + '_val_xe_' + i).val() != 0) {
        recalcInProcess = true;
        $$('#' + prefix + '_val_' + i).val((10 / parseFloat($$('#' + prefix + '_val_xe_' + i).val().replace(/\,/g, '.'))).toFixed(0));
        recalcInProcess = false;
    }
}
