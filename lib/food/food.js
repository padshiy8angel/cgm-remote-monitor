...
client.init(function loaded () {
  ...
  function drawQuickpick() {
    ...
    $('.fe_recalc').change(reCalcPortions);
  ...
  function reCalcPortions(event) {
    var index = $(this).attr('index');
    var findex = $(this).attr('findex');
    var newKoeff = parseFloat($(this).val().replace(/\,/g,'.')) / foodquickpick[index].foods[findex].portions;
    $.each(foodquickpick[index].foods, function(index, value){
      if (value.unit == foodunits[0]) {
        value.portions = Math.round(value.portions * newKoeff * 100) / 100;
      }
    });
    calculateCarbs(index);
    drawQuickpick();
