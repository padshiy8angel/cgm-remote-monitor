...
daytoday.html = function html(client) {
  ...
  var ret =
    ...
    + '&nbsp;'+translate('Show charts')
    + ' <select id="rp_ddchart" name="sel_ddchart">'
    + '  <option value="0">'+translate('No')+'</option>'
    + '  <option value="1" selected>'+translate('Yes')+'</option>'
    + '</select>'
    + '&nbsp;'+translate('Size')
    + ' <select id="rp_size">'
...
daytoday.prepareHtml = function daytodayPrepareHtml(sorteddaystoshow) {
  $('#daytodaycharts').html('');
  var showDD = $('#rp_ddchart').val() == '1';
  sorteddaystoshow.forEach(function eachDay(d) {
    var ddchart = showDD ? '<td><div id="daytodaystatchart-' + d + '"></td>' : '';
    $('#daytodaycharts').append($('<table><tr><td><div id="daytodaychart-' + d + '"></div></td>' + ddchart + '</tr></table>'));
  });
};
...
//            .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (t.absolute ? Number(t.absolute).toFixed(2) + 'U' : ''));
    ...
    data.treatments.forEach(function (treatment) {
      ...
      if (treatment.carbs && options.carbs) {
        ...
        var insCarb = 0;
        if (ic != null && ic > 0) {
          insCarb = (treatment.carbs/ic).toFixed(2);
        }
        context.append('rect')
        ...
        context.append('text')
          ...
          .text(treatment.carbs+' g ('+insCarb+'U)');
      }
