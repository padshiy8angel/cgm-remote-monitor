...
treatments.html = function html(client) {
    ...
    var ret =
                    ...
                    + '<label><input type="checkbox" id="for_doc" name="forDoc">' + translate('for the doctor') + '</label><br/>'
                    + '<label>' + translate('The output frequency of measurements') + '</label>:'
                    + '<select id="freq" name="freq">'
                    + '  <option value=0>' + translate('All') + '</option>'
                    + '  <option value=1>1 ' + translate('minut(-s)') + '</option>'
                    + '  <option value=5>5 ' + translate('minut(-s)') + '</option>'
                    + '  <option value=10>10 ' + translate('minut(-s)') + '</option>'
                    + '  <option value=15 selected>15 ' + translate('minut(-s)') + '</option>'
                    + '  <option value=30>30 ' + translate('minut(-s)') + '</option>'
                    + '  <option value=60>60 ' + translate('minut(-s)') + '</option>'
                    + '</select>'
...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
    ...
    function deleteTreatment(event) {
        ...
        var BMdata = JSON.parse(data.boluscalc);
        console.log(BMdata);
    ...
    var table = $('<table>').attr('style', 'fixtable');
    table.append($('<thead>').append($('<tr>').css('background', 'gray')
                    .append($('<th>'))
                    ...
                    .append($('<th>').css('width','50px').attr('align','left').append(translate('GI')))
    ...
    function findClosestSGVToPastTime(time, sgv, minutsCount) {
        var newTime = time + minutsCount;
        var nowData = sgv.filter(function (d) {
            return d.mills <= newTime;
        });
        var focusPoint = _.last(nowData);
        if (!focusPoint || focusPoint.mills + times.mins(15).mills < newTime) {
            return null;
        }
        return focusPoint;
    }

    function pushNewTreatments(treatments, newItem) {
        if (newItem != null) {
            var newTr = {
                glucose: newItem.sgv,
                glucoseType: "Sensor",
    ...
    sorteddaystoshow.forEach(function (day) {
        ...
        var sgv = _.clone(datastorage[day].sgv);
        var lastDate = 0;
        var ONE_MIN_IN_MS = 60000;
        var forDoc = $('#for_doc').is(':checked');
        if (forDoc) {
            var firstSgv = _.first(sgv);
            // Первый замер за день (около 00:00)
            pushNewTreatments(treatments, firstSgv);
            // Замер в 3.00
            pushNewTreatments(treatments, findClosestSGVToPastTime(firstSgv.mills, sgv, ONE_MIN_IN_MS * 60 * 3));
            // Замер в 6.00
            pushNewTreatments(treatments, findClosestSGVToPastTime(firstSgv.mills, sgv, ONE_MIN_IN_MS * 60 * 6));
            for (var k = 0; k < treatments.length; k++) {
                var treat = treatments[k];
                if (treat.insulin != null && treat.insulin != '') {
                    // 1.5 часа после укола
                    pushNewTreatments(treatments, findClosestSGVToPastTime(treat.mills, sgv, ONE_MIN_IN_MS * 60 * 1.5));
                    // 2 часа после укола
                    pushNewTreatments(treatments, findClosestSGVToPastTime(treat.mills, sgv, ONE_MIN_IN_MS * 60 * 2));
                    // 3 часа после укола
                    pushNewTreatments(treatments, findClosestSGVToPastTime(treat.mills, sgv, ONE_MIN_IN_MS * 60 * 3));
                    // 4 часа после укола
                    pushNewTreatments(treatments, findClosestSGVToPastTime(treat.mills, sgv, ONE_MIN_IN_MS * 60 * 4));
                }
            }
        } else {
            var periodShow = parseInt($('#freq').val());
            var perCalc = periodShow != 0 ? ONE_MIN_IN_MS * periodShow - 1 : 1;
            console.log('periodShow != 0 ? ONE_MIN_IN_MS * periodShow - 1 : 1 = ' + perCalc);
            sgv = sgv.filter(function (d) {
                var ok = (lastDate + perCalc) < d.mills;
                if (ok) {
                    lastDate = d.mills;
                }
                return ok;
            });
            for (var i = 0; i < sgv.length; i++) {
                pushNewTreatments(treatments, sgv[i]);
            }
        }
        ...
        for (var t = 0; t < treatments.length; t++) {
            ...
            if (bc != null) {
                if (bc.foods != null) {
                    tr.notes = '';
                    for (var j = 0; j < bc.foods.length; j++) {
                        var foodItem = bc.foods[j];
                        if (foodItem != null) {
                            var foodItemUnitName = '';
                            ...
                            var prsStr = (foodItem.unit == 'g' ? Math.round(foodItem.portion * foodItem.portions * 100) / 100 : foodItem.portions);
                            prsStr += ' ' + foodItemUnitName;
                            tr.notes += foodItem.name + ' ' + prsStr + ' (' + Math.round(foodItem.carbs * foodItem.portions * 100) / 100 + ' гу)' + '<br>';
                        }
                    }
                }
            }
            if (tr.glucose == null || tr.glucose == '') {
                var cgvItem = findClosestSGVToPastTime(tr.mills, sgv, 0);
                if (cgvItem != null) {
                    tr.glucose = cgvItem.sgv;
                    tr.glucoseType = 'Sensor';
                }
            }
            table.append($('<tr>').addClass('border_bottom')
                            .append($('<td>')
                                    .append($('<img>').addClass('deleteTreatment').css('cursor', 'pointer')
                                            .attr('title', translate('Delete record')).attr('src', icon_remove)
                                            .attr('data', JSON.stringify(tr)).attr('day', day))
                                    .append('&nbsp;')
                                    .append($('<img>').addClass('editTreatment').css('cursor', 'pointer')
                                            .attr('title', translate('Edit record')).attr('src', icon_edit)
                                            .attr('data', JSON.stringify(tr)).attr('day', day))
                    )
                            ...
                            .append($('<td>').attr('align','center').attr('style','font-style: italic;').append(bc != null && bc.gi_real != null && !forDoc ? bc.gi_real : ''))
