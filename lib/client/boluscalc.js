...
function init (client, $) {
  ...
  function setSensorBG(sgv, selectedTime) {
    if (sgv != null) {
      var sensorbg = sgv.mgdl;
      if (sensorbg < 39) {
        sensorbg = 0;
      } else {
        sensorbg = client.utils.scaleMgdl(sensorbg);
      }
      if (selectedTime.getTime() - sgv.mills > 16 * 60 * 1000) {
        boluscalc.oldbg = true; // Do not use if record is older than 15 min
        sensorbg = 0;
      }
      return sensorbg;
    } else {
  ...
  function setBG (sgv, selectedTime) {
    ...
    var sgvByTime = findClosestSGVToPastTime(selectedTime);
    if (sgv != null) {
      sensorbg = setSensorBG(sgv, selectedTime);
    } else {
      sensorbg = setSensorBG(findClosestSGVToPastTime(selectedTime), selectedTime);
    }
    // Set BG
  ...
  boluscalc.updateVisualisations = function updateVisualisations (sbx) {
    // update BG in GUI
    setBG($('#bc_nowtime').is(':checked') ? sbx.lastSGVEntry() : null, mergeDateAndTime().toDate());
  ...
  function setPreBolusManual () {
    $('#bc_preBolus_Manual').val(parseInt($(this).val()));
  }
  ...
  boluscalc.prepare = function prepare () {
    ...
    $('#bc_usecob').prop('checked', 'checked');
    $('#bc_useiob').prop('checked', 'checked');
    $('#bc_bgfromsensor').prop('checked', 'checked');
    $('#bc_carbs').val('');
    $('#bc_quickpick').val(-1);
    $('#bc_preBolus').change(setPreBolusManual);
    $('#bc_preBolus').val(0);
    $('#bc_preBolus_Manual').val(0);
    $('#bc_notes').val('');
    ...
    $('#bc_readyMealWeight').change(calcReadyMeal);
    $('#bc_readyMealWeight').val(100);
    $('#bc_wfc').change(calcWfc);
    $('#bc_wfc').val(10);
    $('#bc_carbs').change(calcReadyMeal);
    setDateAndTime();
    boluscalc.eventTimeTypeChange();
    boluscalc.updateVisualisations(client.sbx);
    boluscalc.calculateInsulin();
  ...
  function calcWfc (event) {
    console.log('calcWfc')
    var carbs = $('#bc_carbs').val() > 0 ? $('#bc_carbs').val() : 1;
    var readyMealWeight = $('#bc_readyMealWeight').val();
    var wfc = $('#bc_wfc').val() > 0 ? $('#bc_wfc').val() : 10;
    $('#bc_wfc_total').text((readyMealWeight * wfc / carbs).toFixed(0));
  }

  function calcReadyMeal (event) {
    console.log('calcReadyMeal')
    var carbs = $('#bc_carbs').val();
    var readyMealWeight = $('#bc_readyMealWeight').val() > 0 ? $('#bc_readyMealWeight').val() : 100;
    $('#bc_readyMeal100Carbs').text((100 * carbs / readyMealWeight).toFixed(1));
  }
  ...
  boluscalc.updateGui = function updateGui (record) {
    ...
    if (record.foods.length) {
      ...
      for (var fi = 0; fi < record.foods.length; fi++) {
        ...
        var st_gi_real = '';
        if (f.gi_real <= 40) {
          st_gi_real = 'background-color:green;color:white';
        } else if (f.gi_real > 40 && f.gi_real <= 70) {
          st_gi_real = 'background-color:yellow;color:black';
        } else {
          st_gi_real = 'background-color:red;;color:white';
        }
        html += '<td style="' + st_gi_real + '">'+ translate('GI') + ' '+ f.gi_real + '</td>';
        html += '</tr>';
      }
      html += '</table>';
      $('#bc_food').html(html);
      ...
      $('#bc_gi_real').text(record.gi_real);
      if (record.gi_real <= 40) {
        $('#bc_gi_real').css('background-color', 'green');
        $('#bc_gi_real').css('color', 'white');
      } else if (record.gi_real > 40 && record.gi_real <= 70) {
        $('#bc_gi_real').css('background-color', 'yellow');
        $('#bc_gi_real').css('color', 'black');
      } else {
        $('#bc_gi_real').css('background-color', 'red');
        $('#bc_gi_real').css('color', 'white');
      }
    } else {
      $('#bc_food').html('');
  ...
  boluscalc.gatherBoluscalcData = function gatherBoluscalcData () {
    ...
    if (targetBGLow === 0 || targetBGHigh === 0 || isf === 0 || ic === 0) {
      ...
      boluscalc.record = {};
      console.log('error: targetBGLow (' + targetBGLow + ') || targetBGHigh (' + targetBGHigh + ') || isf (' + isf + ') || ic (' + ic + ') = 0');
      return;
    } else {
      $('#bc_inzulinbgtd').css('background-color', '');
    }
    ...
    if (record.foods.length) {
      ...
      for (var fi = 0; fi < record.foods.length; fi++) {
        ...
        // 50 - def gi if not selected
        gisumreal += f.carbs * f.portions * (f.gi_real != null ? f.gi_real : 50);
      }
      record.gi = (gisum / record.carbs).toFixed(2);
      record.gi_real = (gisumreal / record.carbs).toFixed(2);
    } else {
      record.gi = $('#bc_gi').val();
    }
  ...
  function gatherData () {
    ...
    if (!data.boluscalc) {
      window.alert('Calculation not completed!');
      return null;
    }
    ...
    var selPreBolus = parseInt($('#bc_preBolus').val());
    var inpPreBolus = parseInt($('#bc_preBolus_Manual').val());
    data.preBolus = inpPreBolus != 0 ? inpPreBolus : selPreBolus;
    data.notes = $('#bc_notes').val();

    if ($('#bc_othertime').is(':checked') || data.eventTime == null ) {
      data.eventTime = mergeDateAndTime().toDate();
    }

    // replace boluscalc.eventTime by ISO string
    data.boluscalc.eventTime = data.boluscalc.eventTime != null ? data.boluscalc.eventTime.toISOString() : data.eventTime.toISOString();
    return data;
  }
  ...
  function confirmPost (data) {
    if (window.confirm(buildConfirmText(data))) {
      $.ajax({
        ...
        console.info('treatment saved', response);
        window.alert(translate('Entering record failed') + '. ' + translate('Status') + ': ' + response.status);
      });
  ...
  function quickpickChange (event) {
    ...
    if (qpiselected != null && qpiselected != '-1') { // (none)
      ...
      var tmpFood = _.cloneDeep(qp.foods);
      $.each(tmpFood, function(index, value) {
        var addedPortion = false;
        $.each(foods, function(idx, val) {
          if (val._id == value._id) {
            val.portions = parseFloat(val.portions) + parseFloat(value.portions);
            addedPortion = true;
          }
        });
        if (!addedPortion) {
  ...
  var recalcInProcess = false;

  function recalcCarbsByPortion (event) {
    if (!recalcInProcess) {
      ...
      if (index !== null && !isNaN(portions) && portions > 0) {
        $('#bc_addcarbs').val(Math.round(foodlist[index].carbs * portions * 100) / 100);
      }
      recalcInProcess = false;
    }
  }

  function recalcPortionByCarbs (event) {
    if (!recalcInProcess) {
      recalcInProcess = true;
      var index = $('#bc_data').val();
      var carbs = parseFloat($('#bc_addcarbs').val().replace(',', '.'));
      if (index !== null && !isNaN(carbs) && carbs > 0) {
        var itemFood = foodlist[index];
        $('#bc_addportions').val(Math.round((carbs / itemFood.carbs) * 100) / 100);
      }
      recalcInProcess = false;
    }
  }
...
$('#bc_data').change(recalcCarbsByPortion);
    $('#bc_addportions').change(recalcCarbsByPortion);
    $('#bc_addcarbs').change(recalcPortionByCarbs);
    $('#bc_addportions').val('1');
    $('#bc_addfooddialog').dialog({
      ...
      , buttons: [
        ...
        { text: translate('Recalc by carbs'),
          class: 'leftButton',
          click: recalcPortionByCarbs
        }
        , {
  ...
  function findClosestSGVToPastTime (time) {
    ...
    if (!focusPoint || focusPoint.mills + times.mins(15).mills < time.getTime()) {
      return null;
    }
    return focusPoint;
  }
