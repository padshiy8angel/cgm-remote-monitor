...
client.init = function init (callback) {
  ...
  var finalDate = (new Date()).getTime() + 5 * 60 * 60 * 1000;
  var divCounter = $("#refreshCounter");
  if (divCounter != null) {
    try {
      divCounter.countdown(finalDate, function (event) {
        $(this).text('Refresh: ' + event.strftime('%H:%M:%S'));
...
client.load = function load (serverSettings, callback) {
  ...
  client.ylineval = d3.select('body').append('div')
          .attr('id', 'ylineval')
          .attr('class', 'tooltip')
          .style('opacity', 0);
  ...
  function prepareEntries () {
    ...
    var tooOld = client.now - times.hours(24*10).msecs;
