...
hashauth.init = function init(client, $) {
    ...
    if (!hashauth.apisecret || hashauth.apisecret.length < 12) {
      ...
      hashauth.verifyAuthentication( function(isok) {
        if (isok) {
          ...
          window.alert(translate('Wrong API secret'));
          if (callback) {
            callback(false);
          }
        }
  ...
  hashauth.inlineCode = function inlineCode() {
    ...
    return '<div id="requestauthenticationdialog" style="display:none" title="' + translate('Device authentication') + '">' +
