  ...
  function upsert (obj, fn) {
    ...
    api( ).update(query, obj, {upsert: true}, function complete (err, updateResults) {
      ...
      if (!err && obj.preBolus) {
        ...
        if (pbTreat.notes != null && pbTreat.notes != '') {
          pbTreat.notes += '\n';
        } else {
          pbTreat.notes = '';
        }
        pbTreat.notes += translate('Pause') + ' '  + obj.preBolus + ' ' + translate('Min');
        //pbTreat.notes += 'Pause '  + obj.preBolus + ' Min';
        console.log(pbTreat.notes);
